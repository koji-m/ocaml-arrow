let ceil n d =
  if n mod d > 0 then n / d + 1 else n / d
...
module Bitchunk = struct
  ...
  let of_buffer (buf : Buffer.t) offset len =
    ...
    {
      buffer = Buffer.sub buf ~off:byte_offset ~len:(Buffer.length buf - byte_offset);
  ...
  let iteri f bc =
    ...
    let rec iteri_ f i =
      if i < chunk_len then
        ...
        let c = (current lsr bit_offset) lor (255 land (next lsl (8 - bit_offset))) in
        f i (Char.chr c);
        iteri_ f (i + 1)
      else
