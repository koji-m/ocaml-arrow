...
and Field : sig
    ...
    module Message : sig
        val build_field : FbMessageRt.Builder.t -> Field.t -> FbMessage.Field.t FbMessageRt.wip
    end

    module File : sig
        val build_field : FbFileRt.Builder.t -> Field.t -> FbFile.Field.t FbFileRt.wip
    end
    ...
    module Message = struct
        let get_i32_field_type b =
            ...
            let i32_type = FbMessage.Int.Builder.(
                start b
                |> add_is_signed true
                |> add_bit_width 32l
                |> finish
        ...
        let get_utf8_field_type b =
            let children = FbMessage.Field.Vector.create b [||] in
            let utf8_type = FbMessage.Utf8.Builder.(
                start b |> finish
            ) in
            (utf8_type, children)
        ...
        let rec get_int32_list_field_type b child =
            ...
            let list_type = FbMessage.List.Builder.(
                start b |> finish
            ) in
            (list_type, children)
        and get_struct_field_type b children =
            ...
            let struct_type = FbMessage.Struct_.Builder.(
                start b |> finish
            ) in
    ...
    module File = struct
        let get_i32_field_type b =
            ...
            let i32_type = FbFile.Int.Builder.(
                start b
                |> add_is_signed true
                |> add_bit_width 32l
                |> finish
        ...
        let get_utf8_field_type b =
            let children = FbFile.Field.Vector.create b [||] in
            let utf8_type = FbFile.Utf8.Builder.(
                start b |> finish
            ) in
            (utf8_type, children)
        ...
        let rec get_int32_list_field_type b child =
            ...
            let list_type = FbFile.List.Builder.(
                start b |> finish
            ) in
            (list_type, children)
        ...
        and get_struct_field_type b children =
            ...
            let struct_type = FbFile.Struct_.Builder.(
                start b |> finish
            ) in
            (struct_type, children)
