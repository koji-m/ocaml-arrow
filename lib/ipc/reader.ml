...
module Array_reader = struct
  ...
  let next_buffer ar =
    ...
    | Some (off, len) -> Some (Buffer.sub ar.data ~off:(Int64.to_int off) ~len:(Int64.to_int len))
    | None -> None
...
let create_array reader field =
  ...
  | Datatype.Int32 ->
    let null_buffer = match Array_reader.next_buffer reader with
    | Some buf -> buf
    | None -> raise (Unexpected "null buffer not found") in
    let data_buffer = match Array_reader.next_buffer reader with
    | Some buf -> buf
    | None -> raise (Unexpected "data buffer not found") in
...
let read_record_batch buf (b, rb) ver schema_ =
  let bufs = FbMessage.RecordBatch.buffers b rb
    |> FbMessageRt.Option.get
    |> FbMessage.Buffer.Vector.to_seq b
    |> Seq.map (fun elt -> (FbMessage.Buffer.offset b elt, FbMessage.Buffer.length b elt)) in
  let nodes = FbMessage.RecordBatch.nodes b rb
    |> FbMessageRt.Option.get
    |> FbMessage.FieldNode.Vector.to_seq b
    |> Seq.map (fun elt -> FbMessage.FieldNode.((length b elt, null_count b elt))) in
  let array_reader = Array_reader.({
    version = ver;
    data = buf;
    nodes = Seq.to_dispenser nodes;
    buffers = Seq.to_dispenser bufs;
  }) in
  let children = Array.map (fun f -> create_array array_reader f) (Schema.fields schema_) in
...
let fb_to_field b fb_field =
  ...
  let name_ = FbMessage.Field.name b fb_field
    |> FbMessageRt.Option.get
    |> FbMessageRt.String.to_string b in
  match f_type with
  | "int" -> Field.({type_ = Datatype.Int32; name = name_})
  | _ -> raise Datatype.NotSupported

let fb_to_schema b fb_schema =
  let fields_ = FbMessage.Schema.fields b fb_schema
    |> FbMessageRt.Option.get
    |> FbMessage.Field.Vector.to_array b
    |> Array.map (
      fun elt -> fb_to_field b elt) in
...
module StreamReader = struct
  ...
  let create reader =
    ...
    let _ = if meta_size = continuation_marker then
      Reader.read_exact reader meta_size
    else
      0 in
    ...
    let (Root (b, msg)) = FbMessage.Message.root ~size_prefixed:false Flatbuffers.Primitives.Bytes meta_buffer in
    let schema = FbMessage.Message.header
        ~schema:(fun schm ->
          fb_to_schema b schm
        )
  ...
  let next sr =
    ...
    if Reader.read_exact sr.reader meta_buffer > 0 then
      let (Root (b, msg)) = FbMessage.Message.root ~size_prefixed:false Flatbuffers.Primitives.Bytes meta_buffer in
      let header_type = FbMessage.(Message.header_type b msg |> MessageHeader.to_string) in
