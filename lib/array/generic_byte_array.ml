...
let of_string_array a =
  ...
  let offsets =
    Array.map (fun s -> match s with Some s -> String.length s | None -> 0) a
    |> Offset_buffer.from_length
  in
  let values =
    Array.fold_left
      (fun acc s -> match s with Some s -> acc ^ s | None -> acc)
      "" a
    |> Buffer.of_string
  in
  let nulls = Null_buffer.create num_slots in
  Array.iteri
    (fun i s -> match s with Some _ -> Null_buffer.set nulls i | None -> ())
    a;
  {
