...
let test_bit_sub () =
  ...
  Alcotest.(check char) "same char 0"
    (Buffer.get buf_expect 0)
    (Buffer.get buf_test 0);

  Alcotest.(check char) "same char 1"
    (Buffer.get buf_expect 1)
    (Buffer.get buf_test 1)

let () =
  run "Arrow_buffer" [
    "basic tests", [
      test_case "bit_sub works" `Quick test_bit_sub;
    ];
  ]
